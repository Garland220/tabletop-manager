...
$(document).ready(function(){
	if ( $("#droplowest").is(':checked') )
		toggle($("#droplowest"), $("#dropValue"));
});

function toggle( from, target ){
	if ($(from).is(':checked')){
		...
		$(target).removeClass("text");
	}
}

function getSound( rollsize )
{
	if ( rollsize )
		return "sounds/multidice.wav";
	else
...
function doRoll( dice, power ){
	...
	if ( $("#usesound").is(':checked') )
		$.fn.soundPlay({url: getSound(largeRoll), playerId: 'embed_player', command: 'play'});
	return false;
}
